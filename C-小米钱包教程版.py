'''
脚本分享本次分享的是原始脚本
本laizhi：yaohuoluntan by将军
问题咨询回答都可以找qq群：940995049
每日7,19点自动完成任务获取会员时长，到达30天可兑换热门app视频会员一个月，完成每天两个视频任务获取会员时长
定时规则 39 7,19 * * *
#抓取https://m.jr.开头的包请求头的cookie
1:本脚本存在cookie时效问题3天一抓

#群内小米经过羽化大佬优化了扫码登录，优化了cookie只有一天的有效期，扫码登录一次可持续运行
#变量XIAOMIQIANBAO 多账号回车键换行隔行
打开抓包软件，再去小米钱包app抓包，地址是 https://m.jr.开头得包进去找到请求头中得cookie就是了
...
if __name__ == "__main__":
    cookie = ''
    if not cookie:
        cookie = os.getenv('XIAOMIQIANBAO', '')
