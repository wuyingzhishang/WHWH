"""
name: 漫鸟旧衣服上门回收,2.5起提
Author: MK集团本部
Date: 2024-09-24
export ldhb="authorization"
cron: 0 5 * * *
"""
#import notify
...
def userinfo(wx, authorization):
	...
	try:
		...
		if "操作成功" in info["message"]:
			return info["data"]["base_info"]["userPhone"],info["data"]["direct_balance"]
...
def run(wx,authorization):
	...
	try:
		...
		print("📱：账号已过期或异常")
def main():
	if os.environ.get("mnhs"):
		ck = os.environ.get("mnhs")
	else:
		ck = ""
		if ck == "":
			print("请设置变量")
